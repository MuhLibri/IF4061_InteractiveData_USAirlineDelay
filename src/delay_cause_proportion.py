...
def delay_cause_stacked_bar(df: pd.DataFrame, selected_years: list[str]):
    st.markdown("<h2 style='font-size: 24px;'>Delay Cause per Year</h2>", unsafe_allow_html=True)
    ...
    fig.update_layout(
        barmode='stack',
        xaxis=dict(title="Airline Year", tickangle=45),
